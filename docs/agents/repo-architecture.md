...
### Public numerical Process eligibility

Published numerical Process eligibility is exactly state `100`. The reserved publication segment `101..199` grants no numerical eligibility, and the published derived Result state `120` is never a root, provider, demand target, or matrix axis. `_shared/lca_snapshot_scope.ts` owns `PUBLIC_NUMERICAL_PROCESS_STATES` and the `NUMERICAL_SNAPSHOT_POLICY_VERSION` marker (`public-numerical-state-100-excluding-result-120:v1`), whose literal is owned by Worker `solver_worker::NUMERICAL_SNAPSHOT_POLICY_VERSION`. Edge uses that marker for snapshot lookup identity, request-hash identity, and stored ready evidence admission: it is not a request field, and Worker records the field `numerical_policy_version` in its own snapshot build config. Edge does not restate Database or Worker eligibility-predicate versions that it does not consume. `DEFAULT_PUBLISHED_PROCESS_STATES` survives only so already deployed `100..199`-shaped filters still parse and can be named by `isRetiredNumericalStateArgument` as filters that must never be reused for a new calculation.

Every Edge snapshot producer — `current_user`, `open_data`, `all_data`, `public_plus_owner_draft` — builds a filter whose `process_states` is exactly `[100]`, and every one of them still carries an explicit owner clause. The non-versioned `current_user`/`open_data`/`all_data` filters send `include_user_id` without `include_user_state_codes`; Worker materializes those as `state_code = 100 OR (user_id = actor AND state_code = 0)`, so `buildSnapshotVisibilityOrExpression` must bind the owner branch to an explicit owner state code rather than an owner-all-states branch. `_shared/lca_process_scope.ts` applies the same rule to root and demand admission: `open_data` admits only exact `100`; `all_data` admits exact `100` plus the actor's own state-`0` draft; `current_user` keeps its own "only the actor's own Process" rule and, within it, admits the actor's numerically eligible `100` row or own state-`0` draft. Owner identity is a visibility fact and never numerical eligibility, so a state-`120` row owned by the requesting actor is rejected exactly like a foreign one, and the review state `20` stays reachable only through its existing dedicated Review Admin quality diagnostic. When the rejected row is the published Result state, `validateProcessEntriesInDataScope` returns the locatable `published_result_process_is_not_a_numerical_input` reason, aligned with the Worker rejection vocabulary, instead of a generic scope error; foreign owner drafts keep their existing generic error.

The v2 `public_plus_owner_draft` scope manifest keeps its agreed semantics: it admits exactly public state `100` plus every actor-owned state-`0` row irrespective of `team_id`/`review_id` workflow metadata, and rejects `120` and every other owner state.

`rejectNonNumericalSnapshotProcessFilter` validates the **raw stored** `processFilter`, not a normalized one, because `parseSnapshotProcessFilter` is lossy: an absent, non-array, or empty state list becomes `[]`, which a normalized check would admit as numerical. It therefore requires an object, an explicit boolean `all_states: false`, and a non-empty integer-array `process_states`, and fails closed with `snapshot_process_filter_missing`, `snapshot_process_filter_malformed`, or `snapshot_process_states_missing` otherwise. It then rejects `all_states: true`, state `120`, the review-diagnostic state `20`, the retired `100..199` membership, and any other non-`100` published state. The JSON array form is the only accepted state-list shape because both writers of `private.lca_network_snapshots` — Worker `snapshot_builder` and the Database `api.svc_lca_snapshot_build_enqueue` draft insert — write arrays; the comma-separated string form belongs to the Worker payload contract and is not accepted here.

### Stored-snapshot evidence contract

A stored ready snapshot backs a new calculation only when it declares the current numerical policy. This is an explicit cross-repo contract over the existing transport, not a Database field or RPC change:

- **Worker #289 (producer, assigned separately):** `upsert_snapshot_artifact_row` writes `numerical_policy_version` into the server-authored **`ready`** `private.lca_network_snapshots.process_filter` object, with the same literal it records in the artifact's `SnapshotBuildConfig`. `api.svc_lca_snapshot_candidates` already returns `processFilter` verbatim, so no new Database field or RPC is required.
- **Edge (consumer, implemented):** the marker read from that existing object field decides whether a stored ready snapshot may back a new calculation.

Two validators keep the two questions separate. `rejectNonNumericalSnapshotProcessFilter` asks "is this proposed filter numerically shaped?" and never requires a marker, because a proposed filter may carry one without proving anything about any stored row. `rejectNonCurrentSnapshotEvidence` asks "does this already-persisted ready snapshot carry current policy evidence?" and requires the marker to be present and current, then applies the filter check. A missing marker (`snapshot_numerical_policy_version_missing`) or a retired one (`snapshot_numerical_policy_version_not_current`) fails closed, and a marker is never inferred from a `100`-shaped `process_states` list — an older owner-all-states filter also stored `process_states: [100]`, which is exactly the regression this keeps closed.

`evaluateSnapshotEvidenceForNewCalculation` applies that decision to a candidate row and also requires a ready artifact URL, so a `draft` enqueue-time row (which stores `coalesce(p_process_filter,'{}')` and can never carry the marker) cannot authorize a ready build. `_shared/lca_snapshot_scope_db.ts` wraps it in the three resolution forms the read paths use, and every one of them returns the accepted candidate only after the decision passes:

- `verifySnapshotEvidenceForNewCalculation` — the explicit-snapshot branch. One candidate lookup returns the accepted row, an absent row reports `matches: false` so callers keep their existing `snapshot_not_ready` handling, and a present-but-non-current row reports the locatable `409` reason.
- `resolveLatestSnapshotForNewCalculation` — the no-user latest-snapshot fallback. It owns the whole fallback, so the snapshot id cannot be observed before the decision passes.
- the per-candidate re-check inside the actor-scoped lookup loop, layered under the `buildSnapshotContainsFilter` containment request.

Callers must reach one of these **before** any cached-result return or enqueue. `test/lca_snapshot_scope_db_test.ts` proves the decisions by calling the helpers with controlled candidate rows; `test/lca_snapshot_evidence_gate_test.ts` only asserts that each entrypoint is wired to them and does not read a latest candidate itself, and it does not prove runtime control flow.

Edge writes the marker in exactly two places: `buildSnapshotContainsFilter`, so `svc_lca_snapshot_candidates` skips pre-policy rows, and the snapshot build request hash. It is deliberately **not** added to `buildSnapshotBuildPayloadFields`, because Worker does not accept it as a caller-supplied payload field. The stored marker the evidence gate consumes is authored by Worker at upsert time, never by Edge.

Historical artifact reads and administrative routes never call the evidence gate, so pre-policy snapshots and their artifacts stay readable. Reading history and accepting an old snapshot as the basis of a new calculation are separate decisions, and only the second one is gated.

Rollout dependency: deploy the compatible Worker producer and build new `ready` snapshots before enabling the Edge consumer gate in an environment. A `ready` snapshot that predates the producer carries no marker and fails closed on all three LCA read paths, so the gate must not be enabled ahead of the producer and its snapshot rebuild.

Support datasets are unaffected: flows, flowproperties, unitgroups, sources, contacts, and lifecyclemodels keep their own state rules, and the TIDAS package export open-data range stays a support-dataset rule rather than a numerical one.
