...
- `lca_solve`: `POST` only.
  ...
  - published numerical Process eligibility is exactly `state_code=100`. The reserved publication segment `101..199` grants no numerical eligibility, and the published derived Result state `120` is never a root, provider, demand target, or matrix axis in any scope, including for the actor who owns that Result
  ...
  - root-process semantics stay distinct: `current_user` accepts only the caller's own numerically eligible processes (`state_code=100`) or own `state_code=0` drafts, `open_data` accepts only published `state_code=100` processes, `all_data` accepts published `state_code=100` plus the caller's own `state_code=0` drafts
  ...
  - a stored snapshot backs a new calculation only when the Worker authored the current numerical-policy marker (`public-numerical-state-100-excluding-result-120:v1`) into its ready `process_filter`. A missing marker returns HTTP `409` `snapshot_numerical_policy_version_missing`; a retired marker returns `snapshot_numerical_policy_version_not_current`; a filter still describing the retired `100..199` membership, an `all_states` universe, state `120`, the review-diagnostic state `20`, or another non-`100` published state returns the matching `snapshot_*_not_numerical` error. The check runs before any cached-result return or job enqueue on `lca_solve`, `lca_query_results`, and `lca_contribution_path`. Historical artifacts stay readable as history; they are simply not reused as new-compute evidence, and a `100`-shaped state list alone is never treated as policy evidence
  - a numerically inadmissible demanded process reports the locatable `reason=published_result_process_is_not_a_numerical_input` when it is the published Result state, instead of a generic scope error
